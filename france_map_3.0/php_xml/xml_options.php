...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
// Liste des villes
$sqlVilles = mysql_query("SELECT `$champVille` FROM `$tableUtilisee` GROUP BY `$champVille`");
if (!$sqlVilles) {
...
$sqlMembres = mysql_query("SELECT `$champNom`, `$champCodePostal`, `$champPays`, `$champId` FROM `$tableUtilisee`ORDER BY `$champNom`");
if (!$sqlMembres) {
	die('Impossible d\'ex&eacute;cuter la requ&ecirc;te : ' . mysql_error());
}
