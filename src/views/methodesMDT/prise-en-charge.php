    <?php $breadcrumb->display()?>
    <div class="container-fluid">
    <h1>Methode MDT: La prise en charge</h1>
        <div id="toc" class="toc">
        </div><!--/.well -->

    <div class="pg01a-prisecharge">

        <h2>Le bilan</h2>
        <h5><strong></strong></h5>
        <div class="bs-callout bs-callout-warning">
            <p>Préambule INDISPENSABLE à tout traitement McKenzie/MDT.<br />C’est de lui que découleront les exercices composant le traitement.</p>
        </div>
        <p>C'est un acte:</p>
        <div class="bs-callout bs-callout-info">
            <ul>
                <li>professionnel.</li>
                <li>de haute qualité.</li>
                <li>reproductible inter-examinateurs.</li>
                <li>sur lequel de nombreux articles de référence internationaux ont démontré la validité et la pertinence.</li>
            </ul>
        </div>

        <h3>L'interrogatoire</h3>
        <p>Il permet de définir:</p>
        <div class="bs-callout bs-callout-info">
                    <ul>
                        <li>vos antécédents.</li>
                        <li>votre état de santé général.</li>
                        <li>les différents traitements (médicaux et non médicaux) entrepris ou en cours.</li>
                        <li>de définir votre problème :
                            <ul class="liste2">
                                <li>localisation (lombaire, dorsale, cervicale ou autre)</li>
                                <li>mode d’apparition</li>
                                <li>ancienneté</li>
                                <li>intensité au cours de la journée</li>
                                <li>impact sur vos activités professionnelles et de loisirs</li>
                            </ul></li>
                    </ul>        </div>



        <h3>L'examen postural</h3>
        <p>Éventuellement bilan neurologique selon la pathologie évaluée</p>
        <h3>L'examen des mouvements répétés</h3>
        <p>Il servira à rechercher la préférence directionnelle, et s’il y a lieu, la centralisation de la douleur.</p>

        <p>Ce bilan permet:</p>
        <div class="bs-callout bs-callout-info">
        <ul>
                <li>la classification dans un des 3 syndromes:
                    <ul>
                        <li>Dérangement</li>
                        <li>Dysfonction</li>
                        <li>Postural</li>
                    </ul>
                </li>
                <li>de proposer à partir de cette classification une stratégie de traitement.</li>
                <li>de repérer d’éventuels « drapeaux rouges » qui seraient des contre-indications à cette prise en charge et pourraient constituer des urgences médicales.</li>
            </ul>
</div>
            <h2>Les séances</h2>
            <p>Le traitement McKenzie/MDT privilégie les techniques d'auto-traitement. Vous serez ainsi amenés à effectuer de façon pluriquotidienne des exercices courts, précis, issus du bilan réalisé préalablement.</p>
            <p>Selon le syndrome que vous présentez, vous aurez à effectuer vos exercices sur une durée allant de quelques jours (syndrome de dérangement) à quelques semaines (syndrome de dysfonction).</p>
            <p>Parfois, en complément des exercices que vous réaliserez quotidiennement, votre praticien pourra effectuer des mobilisations si elles s s’avèrent indispensables à votre progression.</p>
            <p>En aucun cas, lors de cette prise en charge, des techniques de physiothérapie (chaleur, ultra-sons, électrothérapie….) ne sauraient être utilisées.</p>
            <p>La correction posturale est prépondérante et représente une part essentielle du traitement. Dans le syndrome postural, elle constitue même, associée à l’éducation posturale, l’exclusivité du traitement. Dans le syndrome de dérangement, la correction posturale associée à la connaissance et la limitation des mouvements qui vous sont néfastes sont des composantes essentielles du traitement.</p>
            <p>Le traitement McKenzie/MDT permettra en parallèle l’abolition des douleurs, de récupérer les amplitudes articulaires, de connaitre les mouvements néfastes pour vous et d’éviter les récidives.</p>

            <div class="bs-callout bs-callout-warning">
                <p>Grâce à cette prise en charge, vous deviendrez l’acteur principal de votre traitement, vous saurez comprendre les contraintes appliquées à votre dos et  vous serez à même de gérer d’éventuelles récidives, afin d’obtenir des résultats durables.</p>
            </div>

            <h2>Les aides ergonomiques</h2>
            <p>Dans l'approche McKenzie/MDT, l'utilisation d'un coussin ergonomique fait souvent partie intégrante du traitement.</p>
            <p>En effet, comment concilier traitement au cabinet sans prendre en compte ce que le patient peut imposer à sa colonne au cours de la journée chez lui ou lors de son activité professionnelle?</p>
            <p>Dans cette optique et si le bilan initial révèle une prédisposition du patient à la station assise avachie il peut être particulièrement pertinent de corriger cette attitude au quotidien. </p>
            <p>Tous les moyens que le thérapeute jugera nécessaires peuvent être entrepris pour lutter contre une telle attitude:</p>
            <div class="bs-callout bs-callout-info">
                        <ul>
                            <li>conseils pour aménager le poste de travail,</li>
                            <li>strapping proprioceptif de rappel,</li>
                            <li>conseils d'hygiène de vie lombaire.</li>
                        </ul>
                    </div>
            <p>Malgré tout, un des meilleurs moyens de corriger une statique néfaste est de préconiser l'utilisation d'un coussin ergonomique.<br />Cela peut contribuer à:</p>
            <div class="bs-callout bs-callout-info">
                        <ul>
                            <li>renforcer les acquis obtenus par le traitement McKenzie/MDT en cabinet sur le long terme,</li>
                            <li>modifier la posture assise avachie souvent à l'origine de douleurs,</li>
                            <li>éviter les récidives.</li>
                        </ul>

            <p>Il existe de nombreux modèles de coussins lombaires ou cervicaux. De nombreuses marques proposent ces coussins, aussi bien pour la voiture que pour le bureau. Les prix sont souvent assez élevés (entre 50 et 80 euros) et bien souvent  l'acquéreur ne bénéficie pas de conseils pertinents pour leur utilisation.</p>
            <p>L'institut McKenzie International a donc établi un cahier des charges particulièrement pointu concernant ces différentes aides ergonomiques. Son savoir-faire en la matière a intéressé de nombreux industriels : de la conception de véhicules de grande marque avec coussins lombaires conducteur réglables jusqu'à des fabricants de sièges ergonomiques de bureau.</p>
            <p>Ce savoir faire est accessible à tous au travers d'un maillage international de producteurs de produits ergonomiques adaptés répondant à ces normes.</p>
            <h5>N'oubliez pas que ces aides ergonomiques accessibles en ligne seront surtout efficaces si vous suivez les conseils de votre thérapeute quant à leur utilisation et à leur positionnement !</h5>

            <table class="coussin1" style="margin: auto"><tr><td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank"><img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin1_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin5_AFMcK.jpg"></a></td>
                   <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin3_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin4_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin2_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin9_AFMcK.jpg"></a></td></tr>
                   <tr><td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin6_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin7_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin8_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin10_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin11_AFMcK.jpg"></a></td>
                    <td><a href="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/Essentiel Medical_AFMcK.pdf" target="_blank">
                            <img src="<?= Visitor::getRootPage();?>/docs/methode-MDT/prise-en-charge/coussin12_AFMcK.jpg"></a></td></tr>
            </table>
            </div>

        <div style="text-align: center">
            <div class="thumbnail with-caption" style="text-align: center">
                <a href="http://www.essentiel-med.fr/PrestaShop1/">
                    <img src="<?= Visitor::getRootPage()."/docs/members/materiel/logo3 essentiel.png";?>" />
                </a>
                <p style="font-size: 10pt"><a href="http://www.essentiel-med.fr/PrestaShop1/">
                        <i class="glyphicon glyphicon-globe"></i>&nbsp; Essentiel-med
                    </a><small>http://www.essentiel-med.fr/PrestaShop1/</small></p>
            </div>
        </div>

        <p>&nbsp;</p>
            <div class="alert alert-warning">
                <p>Pour les membres adhérents un lien particulier est disponible après avoir entré votre login sur la page dédiée.</p>
            </div>
            <p>&nbsp;</p>            <p>&nbsp;</p>

        </div>


</div></div>
